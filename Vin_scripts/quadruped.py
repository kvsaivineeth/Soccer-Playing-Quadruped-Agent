...
def make_model(floor_size=None, terrain=False, rangefinders=False,
  ...
  xml_string = common.read_model('quadruped_soccer.xml')
...
class Physics(mujoco.Physics):
  ...
  def ball_state(self):
   ...
   # ball_rel_vel = - data.qvel['root'][:3]
    ball_state = np.vstack((ball_rel_pos, ball_rel_vel, ball_rot_vel))
   # ball_state = np.vstack((ball_rel_pos, ball_rel_vel))
...
class Soccer(base.Task):
  """A quadruped task solved by bringing a ball to a goal position."""
